...
> **Note (2026-06-17): the Add job is now a two-cron split.** Heavy isolated
> agent-turns were getting force-aborted in the 01:00–03:00 window — not from a
> cron timeout (the only built-in stalled-session abort floors at 300s and only
> fires on zero-progress turns), but from host/gateway pressure when several
> heavy nightly turns (artist study, autoDream, specimen) pile up on the Mac
> mini during memory consolidation. Short turns survive that pressure; long ones
> (image gen is the long pole) get caught. So the daily procedure below is split
> across two short turns that hand off via the working tree + a gitignored state
> file `data/.daily_prep_state.json`, and both are scheduled into a tight window
> clear of the heaviest non-Axel turns (autoDream 02:00, memory-dreaming 03:00):
>
> | Half | Cron id | Schedule (CST) | Does |
> |------|---------|----------------|------|
> | **PREP** | `2cbd39b0-8396-4f98-b0ab-d2fc361bde20` | `35 1 * * *` (01:35) | Steps 1–4 below minus image: sync, pick species, write data row, fetch photos, write the thumbnail prompt + handoff state. No image, no commit. |
> | **FINISH** | `4d864a43-7d2a-4eec-a96b-820d92144776` | `50 1 * * *` (01:50) | Steps 5–7: read handoff (stale/missing/corrupt guarded), generate thumbnail, run the hard gate, commit atomically, push, announce. |
>
...
> FINISH writes `data/last_daily_run.json` on every exit path at ~01:50, well
